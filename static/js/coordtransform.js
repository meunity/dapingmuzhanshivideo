 ...
 * Updated by FreeGIS on 2016/2/25.
 * 新增了百度经纬度与百度墨卡托互相转换方法
 */
//UMD魔法代码
...
(function(root, factory) {
     ...
     * @returns {*[]}
     */
    var bd_mkt2bd_wgs = function bd_mkt2bd_wgs(lng, lat) {
        var cF = null;
        lng = Math.abs(lng);
        lat = Math.abs(lat);
		...
		var lng = cF[0] + cF[1] * Math.abs(lng);
        var cC = Math.abs(lat) / cF[9];
        var lat = cF[2] + cF[3] * cC + cF[4] * cC * cC + cF[5] * cC * cC * cC + cF[6] * cC * cC * cC * cC + cF[7] * cC * cC * cC * cC * cC + cF[8] * cC * cC * cC * cC * cC * cC;
        lng *= (lng < 0 ? -1 : 1);
        lat *= (lat < 0 ? -1 : 1);
		return [lng,lat];
    };
     ...
     * @returns {*[]}
     */
	var bd_wgs2bd_mkt=function bd_wgs2bd_mkt(lng, lat) {
		var cF = null;
	...
	var getLoop=function getLoop(lng,min,max) {
        while (lng > max) {
            lng -= max - min;
        }
        while (lng < min) {
    ...
    var getRange=function getRange(lat,min,max) {
        if (min != null) {
    ...
    return {
		...
		bd_mkt2bd_wgs:bd_mkt2bd_wgs,
		bd_wgs2bd_mkt:bd_wgs2bd_mkt
    }
}));
